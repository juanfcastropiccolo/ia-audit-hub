...
function ClientDetailPage() {
  ...
  return (
    <main className="p-6 bg-gray-50 min-h-screen dark:bg-gray-900">
      <Button variant="link" onClick={handleBack} className="mb-4 flex items-center">
        ← Volver a Clientes
      </Button>
    ...
    <div className="p-6">
      ...
      <h1 className="text-2xl font-bold mb-4">Cliente: {clientId}</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Árbol de agentes */}
        <div>
          <h2 className="text-xl font-semibold mb-2">Equipo de Auditoría</h2>
          <ul className="ml-4">
            {agents?.rootAgent && renderAgentTree(agents.rootAgent)}
          </ul>
        </div>
        {/* Timeline de eventos */}
        <div>
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto">
            {logs.map(log => (
              <div key={log.id} className="p-3 border-l-4 rounded bg-white shadow-sm"
                style={{ borderColor: log.type === 'error' ? '#f87171' : log.type === 'warning' ? '#fbbf24' : '#60a5fa' }}>
                <div className="flex justify-between text-sm font-medium">
                  <span>{log.agentRole}</span>
                  <span className="text-gray-500">{new Date(log.timestamp).toLocaleTimeString()}</span>
                </div>
                <div className="mt-1 text-sm text-gray-700">{log.content}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {selectedAgent && (
        <div className="mt-6 p-4 bg-gray-50 rounded shadow">
          <h3 className="text-lg font-semibold mb-1">Detalle: {selectedAgent.role}</h3>
          <p className="text-sm text-gray-700">Última acción: {selectedAgent.lastAction}</p>
        </div>
      )}
